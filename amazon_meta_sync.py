...
def load_list_cache(list_id: str) -> dict[str, Any] | None:
    ...
    # Old checkpoints may already contain proven Amazon UI/recommendation ASINs.
    # Remove them in memory before any cached list can contribute memberships.
    cached_asins = {
        normalize_text(value).upper()
        for value in data.get("asins", [])
        if normalize_text(value)
    }
    cached_asins.difference_update(FALSE_MEMBERSHIP_ASINS)
    data["asins"] = sorted(cached_asins)
    return data
    ...
    with sync_playwright() as playwright:
        ...
        for record in approved_records:
            ...
            try:
                ...
                # Final membership gate: no proven false UI ASIN can enter Meta labels,
                # regardless of whether the list came from a fresh scrape or a checkpoint.
                asins = set(asins)
                asins.difference_update(FALSE_MEMBERSHIP_ASINS)
